...
public class MultiMeshIncisionManager : MonoBehaviour
{
    ...
    public void SetHeartCGAL(int MeshIndex)
    {
        MultiMeshAdjacencyList.Instance.ListsUpdate();
        heart = CGAL.CreateMeshObject();
        float[] verticesCoordinate = CGAL.ConvertToFloatArray(MultiMeshAdjacencyList.Instance.MultiMeshWorldPositionVertices[MeshIndex].ToArray());
        CGAL.BuildPolyhedron(heart,
            verticesCoordinate,
            verticesCoordinate.Length / 3,
            MultiMeshManager.Instance.meshes[MeshIndex].triangles,
            MultiMeshManager.Instance.meshes[MeshIndex].triangles.Length / 3);
    }
    ...
    public void DeformCGAL(Vector3 direction, int MeshIndex)
    {
        ...
        Vector3[] newVertices = CGAL.ConvertToVector(CGAL.GetVertices(heart), CGAL.GetNumberOfVertices(heart), GameObject.Find("PartialModel").transform.GetChild(MeshIndex).transform);
    ...
    public void ExecuteDividing(int MeshIndex)
    {
        ...
        foreach (var item in newVertices)
            vertices[item.Key] = MultiMeshManager.Instance.objsTransform[MeshIndex].InverseTransformPoint(item.Value);
    ...
    public void TestGenerateCGAL(int MeshIndex)
    {
        SetHeartCGAL(MeshIndex);
    ...
    public void GenerateIncisionList(int MeshIndex)
    {
        ...
        List<Vector3> worldPosition = MultiMeshAdjacencyList.Instance.MultiMeshWorldPositionVertices[MeshIndex];
        ...
        List<int> leftItems = BFS.Circle(leftSide[currentIndex][leftSide[currentIndex].Count - 1], startVertexPosition, endVertexPosition, zMin, zMax, currentIndex, firstOuterVertexIndex, lastOuterVertexIndex, MeshIndex);
        List<int> rightItems = BFS.Circle(rightSide[currentIndex][rightSide[currentIndex].Count - 1], startVertexPosition, endVertexPosition, zMin, zMax, currentIndex, firstOuterVertexIndex, lastOuterVertexIndex, MeshIndex);
        ...
        if (worldPosition[endPointIndices[currentIndex]].z - MultiMeshManager.Instance.objsTransform[MeshIndex].TransformPoint(MultiMeshManager.Instance.meshes[MeshIndex].normals[endPointIndices[currentIndex]] + MultiMeshManager.Instance.meshes[MeshIndex].vertices[endPointIndices[currentIndex]]).z < 0)
        {
    ...
    public void Extending(int incisionIndex, float currentExtendValue, float oldExtendValue, int MeshIndex)
    {
        ...
        Transform objTransform = MultiMeshManager.Instance.objsTransform[MeshIndex];
    ...
    public void SetDividingListDF(ref bool edgeCheck, int MeshIndex)
    {
        ...
        List<Edge> edgeList = MultiMeshAdjacencyList.Instance.MultiMeshEdgeList[MeshIndex];
        List<Vector3> worldVertices = MultiMeshAdjacencyList.Instance.MultiMeshWorldPositionVertices[MeshIndex];
        startVertexIndex = worldVertices.Count;
        // start
        outerSide = Intersections.MultiMeshTriangleEdgeIntersection(ref outerEdgeIdx, ref outerEdgePoint, startVertexPosition, endVertexPosition, ref outerTriangleIndex, startScreenRay, endScreenRay, edgeList, worldVertices, MeshIndex);
        ...
        // outer 부터 
        int outerTriangleCount = MultiMeshManager.Instance.meshes[MeshIndex].triangles.Length;
        ...
        DivideTrianglesStartIncision(startVertexPosition, outerEdgePoint, startOuterTriangleIndex, outerNotEdgeVertex, outerEdgeIdx, ref outerTriangleCount, false, MeshIndex);
        ...
        while (true)
        {
            ...
            {
                DivideTrianglesEndIncision(endVertexPosition, endOuterTriangleIndex, ref outerTriangleCount, outerEdgeIdx, false, MeshIndex);
            ...
            if (outerSide == 2)
                DivideTrianglesClockWiseIncision(outerEdgePoint, edgeList[outerEdgeIdx].tri1, ref outerTriangleCount, outerEdgeIdx, false, MeshIndex);
            else if (outerSide == 1)
                DivideTrianglesCounterClockWiseIncision(outerEdgePoint, edgeList[outerEdgeIdx].tri1, ref outerTriangleCount, outerEdgeIdx, false, MeshIndex);
            else
            {
                ChatManager.Instance.GenerateMessage(" 자를 수 없는 Edge 입니다.");
                edgeCheck = true;
